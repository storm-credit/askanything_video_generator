...
async def generate_video_v2(req: GenerateRequest):
    ...
    if req.channel:
        _ch_preset = get_channel_preset(req.channel)
        if _ch_preset:
            if tts_speed == 0.9 and _ch_preset.get("tts_speed"):
...
class BlenderRenderRequest(BaseModel):
    preset: str = "solar_system"  # "solar_system" | "giant_stars"

@app.post("/api/blender/render")
async def blender_render(req: BlenderRenderRequest):
    """Blender 3D 행성 크기 비교 영상 렌더링."""
    ...
    agent = BlenderAgent(router, tracker)

    messages = []
    async for msg in agent.render_comparison(preset=req.preset):
...
@app.get("/api/blender/presets")
async def blender_presets():
    """사용 가능한 Blender 프리셋 목록."""
    return {
        "presets": [
            {"id": "solar_system", "name": "태양계 크기 비교", "description": "Moon → Earth → Neptune → Jupiter → Sun"},
            {"id": "giant_stars", "name": "별 크기 비교", "description": "Earth → Sun → Sirius → Arcturus → Betelgeuse → UY Scuti"},
        ]
    }
...
@app.post("/api/admin/quota/reset/{project_name}")
