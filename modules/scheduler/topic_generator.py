...
# ── 카테고리 정의 ──
CATEGORIES = [
    "우주/천문", "해양/심해", "지구과학", "생물/진화", "인체/의학",
    "역사/문명", "물리/화학", "기술/공학", "동물행동", "지리/자연현상",
]
...
## 너의 역할
성과 데이터를 분석하고, 다음 주 7일분 토픽(하루 4개 × 7일 = 28개)을 기획한다.
...
## 토픽 배분 규칙 (채널당 3-4개/일)
1. 하루 4개 토픽: 공통 3개 + 시의성/전용 1개 (교대)
   - 홀수일: 시의성 토픽 1개 (공통 — 계절/반복 뉴스 사이클 기반)
   - 짝수일: 전용 1개 (매일 다른 채널 돌아가며)
2. 채널당 하루 최대 4개 (공통3 + 시의성/전용1)
3. 카테고리 배분 (주간 기준):
   - 성과 상위 카테고리: 50%
   - 히트 패턴 복제: 20%
   - 새 카테고리 테스트: 15%
...
## 카테고리 분산 규칙
- 주간 28토픽 중 최소 7개 서로 다른 카테고리 사용
- 같은 카테고리 연속 2일 금지
- 하루 4개 토픽 내 카테고리 겹침 금지
...
## 요청
- 하루 4개 토픽 × {days}일 = {4 * days}개
- 공통 3개 + 시의성/전용 1개 (홀수일=시의성, 짝수일=채널전용)
- 히트 패턴 복제 토픽: 주당 4-5개
- 역사/문화 토픽 주당 최소 3개 포함
- 시의성 토픽은 [시의성]으로 표시
- 출처에 논문명 지어내지 말 것 — 검색 키워드로 검증 가능하게
- 카테고리 분산: 하루 4토픽 내 겹침 금지, 주간 최소 7개 카테고리
"""
