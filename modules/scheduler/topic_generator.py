...
## 토픽 배분 규칙
1. 하루 6개 토픽: 공통 4개 + 전용 1개 + 트렌딩 1개
2. 전용 토픽은 매일 다른 채널에 돌아가며 배정
3. 트렌딩 토픽: 최근 1주일 이내 과학/발견/우주 뉴스 기반 (공통 또는 특정 채널)
4. 카테고리 배분 (주간 기준):
...
## 요청
- 하루 6개 토픽 × {days}일 = {6 * days}개
- 공통 4개 + 전용 1개 + 트렌딩 1개 (최신 과학/발견 뉴스)
- 히트 패턴 복제 토픽: 주당 7-8개 (Top 영상과 같은 카테고리+훅, 다른 소재)
