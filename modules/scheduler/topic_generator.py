...
## 너의 역할
성과 데이터를 분석하고, 다음 주 7일분 토픽(하루 5개 × 7일 = 35개)을 기획한다.
...
## 토픽 배분 규칙
1. 하루 5개 토픽: 공통 4개 + 전용 1개
2. 전용 토픽은 매일 다른 채널에 돌아가며 배정
3. 카테고리 배분 (주간 기준):
   - 성과 상위 카테고리: 60% (성과 데이터 참조)
   - 새 카테고리 테스트: 20% (역사/문화, 기술, 물리 등)
   - 채널 강점 카테고리: 20%
...
## 요청
- 하루 5개 토픽 × {days}일 = {5 * days}개
- 공통 4개 + 전용 1개 (전용은 매일 다른 채널)
- 성과 좋은 카테고리 위주 + 새 카테고리 20%
- 역사/문화 토픽 주당 최소 3개 포함 (현재 0%이므로 보강)
- 모든 토픽에 검증 가능한 출처 포함
"""
