...
TIMELY_TAG_RE = re.compile(r"\[시의성\]")
...
def _build_obsidian_uri(file_path: str) -> str:
    """옵시디언 파일 열기 URI 생성."""
    vault_path = os.path.abspath(DEFAULT_VAULT_PATH)
    vault_name = os.path.basename(vault_path.rstrip("\\/"))
    relative_name = os.path.basename(file_path)
    return f"obsidian://open?vault={quote(vault_name)}&file={quote(relative_name)}"
...
def parse_day_file(file_path: str) -> list[dict[str, Any]]:
    ...
    for section in topic_sections:
        ...
        topic_name_raw = re.sub(r"^(?:✅|☑️|✔️)\s*", "", topic_name_raw).strip()
    ...
    for j in jobs:
        ...
        if key not in groups:
            groups[key] = {
                ...
                "is_timely": j.get("is_timely", False),
